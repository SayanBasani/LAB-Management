...
<p class="mb-1 report-lab">
    <i class="fas fa-vial me-2 text-danger"></i>
    Lab No: '.htmlspecialchars($reportDetails["lab_no"]).'
</p>
...
document.getElementById('searchInput').addEventListener('keyup', function() {
    ...
    reports.forEach(function(report) {
        ...
        let lab = report.querySelector('.report-lab').textContent.toLowerCase();
        let dates = report.querySelectorAll('.report-date');
        let dateText = '';
        dates.forEach(d => dateText += d.textContent.toLowerCase());

        // ✅ Now search by Name OR Lab No OR Dates
        if (name.includes(filter) || lab.includes(filter) || dateText.includes(filter)) {
